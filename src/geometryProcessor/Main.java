...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < shapes.length; i++) {
			for (int n = 0; n < shapes[n].length; n++) {
				if (n % 3 == 0)
					shapes[i][n] = new Shape(Math.random() * 10, "Red", "Circle" + i + n);
				else if (n % 3 == 1)
					shapes[i][n] = new Shape(Math.random() * 10, "Green", "Square" + i + n);
				else if (n % 3 == 2)
					shapes[i][n] = new Shape(Math.random() * 10, "Blue", "Triangle" + i + n);
